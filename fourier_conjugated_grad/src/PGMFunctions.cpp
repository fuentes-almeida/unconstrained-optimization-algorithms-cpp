...
double ErrorFunction(Matrix *ImageX, Matrix *ImageY,double lambda)
{
    ...
    for (int i=0;i<N;i++)
        for (int j=0;j<M;j++)
        {
            ...
            error+=0.5*lambda*pow(ImageX->data[i*M+j]-ImageX->data[((i-1+N)%N)*M+(j+M)%M],2);
            error+=0.5*lambda*pow(ImageX->data[i*M+j]-ImageX->data[((i+N)%N)*M+(j-1+M)%M],2);
            error+=0.5*lambda*pow(ImageX->data[i*M+j]-ImageX->data[((i+1+N)%N)*M+(j+M)%M],2);
            error+=0.5*lambda*pow(ImageX->data[i*M+j]-ImageX->data[((i+N)%N)*M+(j+1+M)%M],2);
        }
    return error;
...
double PixelNeighbours(Matrix *Image, int row, int col)
{
    ...
    sum+=Image->data[((row-1+N)%N)*M+(col+M)%M];
    sum+=Image->data[((row+N)%N)*M+(col-1+M)%M];
    sum+=Image->data[((row+1+N)%N)*M+(col+M)%M];
    sum+=Image->data[((row+N)%N)*M+(col+1+M)%M];
