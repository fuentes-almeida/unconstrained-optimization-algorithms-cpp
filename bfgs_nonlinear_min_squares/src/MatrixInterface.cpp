...
void GaussianElimination(Matrix *A,Vector *b,Vector *x,int n)
{
    vectorFill(x,0);
    for (int k =0;k<n-1;k++){
        for (int i=k+1;i<n;i++){
            double m=A->data[i*n+k]/A->data[k*n+k];
            for (int j=k+1;j<n;j++)
                A->data[i*n+j]=A->data[i*n+j]-m*A->data[k*n+j];
            A->data[i*n+k]=0;
            b->data[i]=b->data[i]-m*b->data[k];
        }
    }
    ...
    for (int i=n-1; i>=0;i--){
        double suma=0;
        for (int j=i+1;j<n;j++)
            suma+=A->data[i*n+j]*x->data[j];
        x->data[i]=(b->data[i]-suma)/A->data[i*n+i];
    }
}
...
void SolveTriInfMatrix(Matrix *MatrixInf,Vector *x,Vector *b,int n)
{
    int i,j;
    double sum=0;
    x->data[0]=b->data[0]/MatrixInf->data[0];
    for(j=1;j<n;j++){
             sum=0;
        for(i=0;i<=j-1;i++){
...
void choleskyDecomposition(Matrix * A)
{
    ...
    for (i=0;i<n;i++)
    {
        sum=0;
        for(k=0;k<i;k++)
            sum+=A->data[i*n+k]*A->data[i*n+k]*A->data[k*n+k];
        A->data[i*n+i]-=sum;
        ...
        for(j=i+1;j<n;j++)
        {
            sum=0;
            for(k=0;k<i;k++)
                sum+=A->data[i*n+k]*A->data[j*n+k]*A->data[k*n+k];
...
double InversePowerMethod(Matrix *A,Vector *curr_vector,int n, double tolerance, int r_max)
{
    int r=0;
    double lamda_curr=0;
    Vector *new_vector;
    ...
    croutDecomposition(A,n);
    vectorNormalize(curr_vector,curr_vector);

    while(r<r_max)
    {
        r++;
        SolveTriInfMatrix(A,new_vector,curr_vector,n);
        SolveTriSupMatrix(A,new_vector,new_vector,n);
        double num=vectorInnerProduct(curr_vector,new_vector);
        double den=vectorInnerProduct(new_vector,new_vector);
        double lamda_new=num/den;
        vectorNormalize(new_vector,new_vector);

        if (fabs(lamda_new-lamda_curr)<tolerance)
        {
            FreeVectorMem(new_vector);
            return lamda_new;
        }
        vectorCopy(new_vector,curr_vector);
